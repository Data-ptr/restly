...
var parameters = function(apicall, opts, files) {
    ...
    for(i in apicall.parameters) {
        ...
        if(param.required && param.type != "file" && (_.isUndefined(opts[i]) || _.isNull(opts[i]) || opts[i].length == 0)) {
            errors.messages.push(i + " is a required parameter");
            errors.errorFields.push(i);
        }
        ...
        if(param.type=='file') {
            ...
            if(param.required && (typeof files == "undefined" || !files[i])) {
                ...
                errors.messages.push(i + " is a requirement");
                errors.errorFields.push(i);
                ...
                if(!matching_file_type) {
                    errors.messages.push(i + " is not a suitable file type. Please see documentation");
                    errors.messages.push(i + " should be of type " + param.file_types.join(", "));
                    errors.errorFields.push(i);
                }
        ...
        if (param.type == "string" && _.isNumber(param.max_length) && !_.isUndefined(opts[i]) && _.isString(opts[i])) {
            ...
            if (opts[i].length > param.max_length) {
                errors.messages.push(i + " exceeds the maximum allowed length ("+param.max_length+")");
                errors.errorFields.push(i);
            }
        ...
        if (param.type == "string" && _.isNumber(param.min_length) && !_.isUndefined(opts[i])) {
            ...
            if (opts[i].length < param.min_length) {
                errors.messages.push(i + " is not valid, a minimum of "+param.min_length+" characters is required");
                errors.errorFields.push(i);
            }
        ...
        if (param.type == "json") {
            if(!_.isUndefined(opts[i]) && (typeof opts[i] != "string" || !isValidJSON(opts[i]))){
                errors.messages.push(i + " is not valid JSON");
                errors.errorFields.push(i);
            }
        }
        ...
        if (param.type == "email" && !_.isUndefined(opts[i]) && opts[i] != "" && !isValidEmail(opts[i])) {
            errors.messages.push(i+" requires a valid email");
            errors.errorFields.push(i);
        }
        ...
        if(param.regex && typeof opts[i] == "string") {
            ...
            if(!rex.exec(opts[i])) {
                errors.messages.push(i + " does not match required format. Please see documentation");
                errors.errorFields.push(i);
            }
        }
        ...
        if(param.type=='enum' && typeof param.values != "undefined" && typeof param.values == "object") {
            if((!_.isUndefined(opts[i])) && param.values.indexOf(opts[i])==-1) {
                errors.messages.push(i + " does not match one of the required values. Please see documentation");
                errors.errorFields.push(i);
            }
        }
        ...
        switch(param.type) {
            ...
            case "int":
                ...
                if(!_.isUndefined(opts[i]) && isNaN(opts[i])) {
                    errors.messages.push(i + " must be an integer");
                    errors.errorFields.push(i);
                }

                // too big?
                if (!_.isUndefined(opts[i]) && param.max && param.max < opts[i]) {
                    errors.messages.push(i + " must be smaller than or equal to "+param.max);
                    errors.errorFields.push(i);
                }

                // too small?
                if (!_.isUndefined(opts[i]) && !_.isUndefined(param.min) && param.min > opts[i]) {
                    errors.messages.push(i + " must be larger than or equal to "+param.min);
                    errors.errorFields.push(i);
                }
            ...
            case "float":
                ...
                if(isNaN(opts[i])){
                    errors.messages.push(i + " must be an number");
                    errors.errorFields.push(i);
                }

                // too big?
                else if (!_.isUndefined(opts[i]) && param.max && param.max < opts[i]) {
                    errors.messages.push(i + " must be smaller than "+param.max);
                    errors.errorFields.push(i);
                }

                // too small?
                else if (!_.isUndefined(opts[i]) && !_.isUndefined(param.min) && param.min > opts[i]) {
                    errors.messages.push(i + " must be larger than "+param.min);
                    errors.errorFields.push(i);
                }
                break;
            ...
            case "date":
                ...
                if(dateCheck.year() == 0) {
                    errors.messages.push(i + " is not an accepted date format. Please see documentation");
                    errors.errorFields.push(i);
                }
                ...
                if(!_.isUndefined(opts[i])) {
                    if(typeof opts[i] != 'boolean') {
                        ...
                        if(vals.indexOf(opts[i])==-1) {
                            errors.messages.push(i + " must be either true or false. Please see documentation");
                            errors.errorFields.push(i);
                        } else {
        ...
        if (param.type == "number collection") {
            if (_.isNumber(param.max_length) && !_.isUndefined(opts[i])){
                if (opts[i].length > param.max_length) {
                    errors.messages.push(i + " exceeds the maximum allowed number of elements ("+param.max_length+")");
                    errors.errorFields.push(i);
                }
            }
        }
        ...
        if (param.type == "url") {
            if(!_.isUndefined(opts[i]) && param.required && (typeof opts[i] != "string" || !isValidWebsite(opts[i]))){
                errors.messages.push(i + " is not a valid url");
                errors.errorFields.push(i);
            }
        }
